"""Capture and validate Issue #705's frozen paired-seed training behavior gate."""
...
from unilab.tools.issue705_training_behavior import (  # noqa: E402
    ARTIFACT_KIND,
    ARTIFACT_PATH,
    BENCHMARK_ID,
    CLAIM_ID,
...
DEFAULT_OUTPUT = Path("/tmp/issue705_training_behavior.json")
...
def _worker_case(plan: TrainingBehaviorPlan, *, seed: int) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix=f"issue705_p7c_seed_{seed}_") as temp_dir:
...
def _run_seed_worker(plan: TrainingBehaviorPlan, *, seed: int) -> dict[str, Any]:
    ...
    case_id = f"behavior-mjwarp_device-seed{seed}"
    with tempfile.TemporaryDirectory(prefix=f"issue705_p7c_worker_{seed}_") as temp_dir:
