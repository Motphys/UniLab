"""Collect the frozen managed MuJoCo/MJWarp rollout G1 MuJoCo baseline.
...
Usage:
    uv run benchmark/env/benchmark_g1_baseline.py --list-cases
    uv run benchmark/env/benchmark_g1_baseline.py --execute
"""
...
DEFAULT_PLAN = Path("tests/acceptance/manager_mjwarp/g1_mujoco_baseline_plan.yaml")
DEFAULT_OUTPUT = Path("tests/acceptance/manager_mjwarp/artifacts/g1_mujoco_phase0_baseline.json")
...
def _run_subprocess(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="unilab_manager_mjwarp_process_") as temp_dir:
...
def _worker_case(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="unilab_manager_mjwarp_worker_") as temp_dir:
...
def _run_ppo_case(plan: G1BaselinePlan, seed: int) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix=f"unilab_manager_mjwarp_ppo_seed{seed}_") as temp_dir:
