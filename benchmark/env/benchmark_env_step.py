...
Usage:
    ...
    uv run benchmark/env/benchmark_env_step.py

    # Single task + backend:
    uv run benchmark/env/benchmark_env_step.py task=g1_walk_flat/motrix
    uv run benchmark/env/benchmark_env_step.py task=g1_walk_rough/mujoco

    # mjwarp backend (Phase 1: g1_walk_flat only) requires extra deps:
    uv run --with mujoco-warp --with warp-lang \\
        benchmark/env/benchmark_env_step.py task=g1_walk_flat/mjwarp

    # Override bench params:
    uv run benchmark/env/benchmark_env_step.py task=go1_joystick_flat/mujoco num_envs=4096 num_steps=500

    # Save to custom locations:
    uv run benchmark/env/benchmark_env_step.py --out-json tmp/env_step.json --plot-dir tmp/env_step_plots
"""
