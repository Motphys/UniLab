...
| G1 WBT 观测历史 | `conf/offpolicy/task/sac/g1_wbt_obs/mujoco.yaml` 中的 `noise_config.obs_history_length` | 为 `gyro`、`joint_pos_rel`、`dof_vel` 与 `last_actions` 提供逐项历史。 |
...
观测宽度是 owner `noise_config` 的函数，硬件运行时不允许猜测。对 G1 WBT owner，
`obs_history_length: 5` 让每个本体感受项携带 5 步历史并按最旧优先展平，而参考项保持
单步。完整的分项顺序见 {doc}`2-g1_whole_body`。
