...
@dataclass(frozen=True)
class BackendCompletionEvent:
    """Opaque backend-native event that does not imply a host synchronization."""
    ...
    def __post_init__(self) -> None:
        object.__setattr__(self, "backend_type", _non_empty(self.backend_type, "backend_type"))
        if not isinstance(self.placement, BufferPlacement):
            raise BackendBatchContractError("completion placement must be a BufferPlacement")
        if self.placement.memory_space is not MemorySpace.DEVICE:
            raise BackendBatchContractError("completion events require device placement")
        if self.handle is None:
            raise BackendBatchContractError("completion event handle cannot be None")
...
class BackendBatchDiagnostics:
    ...
    completion_event: BackendCompletionEvent | None = None
    ...
    def __post_init__(self) -> None:
        ...
        if self.completion_event is not None and not isinstance(
            self.completion_event, BackendCompletionEvent
        ):
            raise BackendBatchContractError(
                "completion_event must be a BackendCompletionEvent or None"
            )
...
def _validate_result_completion(
    ...
    event = diagnostics.completion_event
    if event is None:
        return
    expected = state.plan.state.fields[0].buffer.placement
    if event.backend_type != state.plan.backend_type:
        raise BackendBatchContractError("completion event belongs to a different backend type")
    if event.placement != expected:
        raise BackendBatchContractError(
            "completion event placement does not match the bound backend plan"
        )
