"""Reusable, fail-closed capture primitives for managed MuJoCo/MJWarp rollout phase evidence.
...
class PhaseEvidenceSpec:
    """Immutable capture contract for one managed MuJoCo/MJWarp rollout phase."""
    ...
    def __post_init__(self) -> None:
        if self.issue != 705:
            raise ValueError(
                "managed MuJoCo/MJWarp rollout phase evidence spec must use managed MuJoCo/MJWarp rollout"
            )
