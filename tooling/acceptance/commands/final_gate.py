"""Validate the managed MuJoCo/MJWarp rollout final integration evidence.

Examples:
    uv run scripts/audit_acceptance.py final --head-only --allow-unpromoted
    uv run scripts/audit_acceptance.py final --json
"""
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.json:
        ...
        print(
            "PASS managed MuJoCo/MJWarp rollout final gate "
            f"mode={payload['mode']} promotion_required={payload['promotion_required']}"
        )
    else:
        print(
            "FAIL managed MuJoCo/MJWarp rollout final gate "
            f"mode={payload['mode']} promotion_required={payload['promotion_required']}"
        )
