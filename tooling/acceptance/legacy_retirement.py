"""Fail-closed release evidence for managed MuJoCo/MJWarp rollout legacy-route retirement."""
...
PLAN_FINGERPRINT = "manager_mjwarp-legacy-retirement-v2"
PLAN_PATH = Path("tests/acceptance/manager_mjwarp/legacy_retirement_plan.yaml")
ROLLBACK_PATH = Path("tests/acceptance/manager_mjwarp/legacy_retirement_rollback.yaml")
EVIDENCE_PATH = Path("tests/acceptance/manager_mjwarp/artifacts/phase_7_legacy_retirement.json")
...
ENTRYPOINT_TEST_ID = (
    "tests/integration/test_manager_mjwarp_entrypoints.py::"
...
_EVIDENCE_KIND = "manager_mjwarp-legacy-retirement-evidence-v1"
_ROLLBACK_KIND = "manager_mjwarp-legacy-retirement-rollback-v1"
...
_INPUT_PATHS = (
    ...
    OWNER_PATH,
    Path("scripts/audit_acceptance.py"),
    ...
    Path("tooling/acceptance/legacy_retirement.py"),
    ...
    Path("tests/integration/test_manager_mjwarp_entrypoints.py"),
    Path("tests/integration/test_manager_mjwarp_legacy_retirement.py"),
    Path("tests/manager/test_g1_reference_differential.py"),
    Path("tests/scripts/test_manager_mjwarp_legacy_retirement_audit.py"),
    Path("uv.lock"),
)
    ...
    def finish(self) -> None:
        if self.errors:
            raise LegacyRetirementError(
                f"invalid managed MuJoCo/MJWarp rollout legacy retirement data {self.source}:\n"
...
def _plan_errors(plan: LegacyRetirementPlan) -> list[str]:
    ...
    if (
        entrypoint.contract_fingerprint != "entrypoints-v1"
