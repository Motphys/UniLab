"""Fail-closed support evidence and bidirectional audit for managed MuJoCo/MJWarp rollout.
...
from tooling.acceptance.phase_evidence import sha256_file
...
SUPPORT_EVIDENCE_PATH = Path("tests/acceptance/manager_mjwarp/support_evidence.yaml")
CLAIM_INVENTORY_PATH = Path("tests/acceptance/manager_mjwarp/claim_test_inventory.yaml")
SUPPORT_AUDIT_TEST_ID = (
    "tests/scripts/test_manager_mjwarp_support_audit.py::"
    "test_supported_combinations_have_fresh_bidirectional_evidence"
)
REQUIRED_PHASES = (2, 3, 5, 6)
BENCHMARK_VALIDATOR = "manager_mjwarp_phase5_ppo_v1"
BENCHMARK_ARTIFACT_PATH = Path("tests/acceptance/manager_mjwarp/artifacts/phase_5_mjwarp_ppo.json")
BENCHMARK_TEST_ID = (
...
class SupportEvidenceError(ValueError):
    def __init__(self, source: Path, errors: list[str] | tuple[str, ...]) -> None:
        ...
        super().__init__(
            f"invalid managed MuJoCo/MJWarp rollout support evidence {source}:\n{detail}"
        )
...
def _validate_phase_payload(
    ...
    if phase == 2:
        from tooling.acceptance.phase2 import validate_phase2_evidence

        return validate_phase2_evidence(report, root=root)
    if phase == 3:
        from tooling.acceptance.phase3 import validate_phase3_evidence

        return validate_phase3_evidence(report, root=root)
    if phase == 5:
        from tooling.acceptance.phase5 import validate_phase5_evidence

        return validate_phase5_evidence(report, root=root)
    if phase == 6:
        from tooling.acceptance.phase6 import validate_phase6_evidence

        return validate_phase6_evidence(report, root=root)
    raise ValueError(
        f"no managed MuJoCo/MJWarp rollout phase evidence validator is registered for phase {phase}"
    )
...
def _validate_phase_gates(
    ...
    *,
    ...
    for phase, gate in sorted(by_phase.items()):
        expected_manifest = Path(f"tests/acceptance/manager_mjwarp/manifests/phase_{phase}.yaml")
        expected_artifact = Path(
            f"tests/acceptance/manager_mjwarp/artifacts/phase_{phase}_gate.json"
        )
    ...
    *,
    ...
    for item in support.combinations:
        ...
        if item.backend != "mjwarp":
            errors.append(
                f"{label}: managed MuJoCo/MJWarp rollout support metadata currently accepts only mjwarp"
            )
...
def _validate_benchmark(
    ...
    *,
    ...
    if artifact is None:
        return
    if validate_payload:
        from tooling.acceptance.phase5 import validate_ppo_benchmark_payload
...
def _recommended_rollout_is_verified(root: Path) -> bool:
    try:
        manifest = load_phase_acceptance(
            root / "tests/acceptance/manager_mjwarp/manifests/phase_7.yaml"
        )
...
def audit_support_evidence(
    ...
    *,
    ...
    if support.integration_branch != INTEGRATION_BRANCH:
        errors.append(
            "integration_branch: does not match managed MuJoCo/MJWarp rollout integration branch"
        )
