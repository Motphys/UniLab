"""Capability-derived production task rollout contract for managed MuJoCo/MJWarp rollout.
...
ROLLOUT_PLAN_PATH = Path("tests/acceptance/manager_mjwarp/mjwarp_task_rollout_plan.yaml")
...
class TaskRolloutPlanError(ValueError):
    def __init__(self, source: Path, errors: list[str] | tuple[str, ...]) -> None:
        ...
        super().__init__(
            f"invalid managed MuJoCo/MJWarp rollout task rollout plan {source}:\n{detail}"
        )
