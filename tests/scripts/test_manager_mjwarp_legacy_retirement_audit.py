"""Parser, provenance, and fault audits for managed MuJoCo/MJWarp rollout legacy retirement."""
...
from tooling.acceptance import legacy_retirement as manager_mjwarp_legacy_retirement
from tooling.acceptance.commands import legacy_retirement as audit_manager_mjwarp_legacy_retirement
...
def test_audit_rejects_registry_inheritance_and_fallback_tamper(
    ...
    original_git_blob = manager_mjwarp_legacy_retirement._git_blob

    def tampered_git_blob(root: Path, commit: str, path: Path) -> bytes:
        blob = original_git_blob(root, commit, path)
        if (
            commit == implementation_commit
            and path == manager_mjwarp_legacy_retirement.OWNER_MODULE_PATH
        ):
            source = blob.decode()
    ...
    monkeypatch.setattr(manager_mjwarp_legacy_retirement, "_git_blob", tampered_git_blob)
...
def test_audit_rejects_owner_identity_tamper(monkeypatch: pytest.MonkeyPatch) -> None:
    original_load = manager_mjwarp_legacy_retirement.OmegaConf.load

    def tampered_load(path: Path):
        config = original_load(path)
        if (
            Path(path).resolve()
            == (REPO_ROOT / manager_mjwarp_legacy_retirement.OWNER_PATH).resolve()
        ):
            config.entrypoints.identity.backend = "mujoco"
        return config

    monkeypatch.setattr(manager_mjwarp_legacy_retirement.OmegaConf, "load", tampered_load)
...
def test_legacy_retirement_audit_cli_reports_pass_and_fail(
    ...
    monkeypatch.setattr(
        audit_manager_mjwarp_legacy_retirement,
        "audit_legacy_retirement",
        lambda *args, **kwargs: passing,
    )
    assert audit_manager_mjwarp_legacy_retirement.main([]) == 0
    assert "PASS managed MuJoCo/MJWarp rollout legacy retirement audit" in capsys.readouterr().out
    ...
    assert audit_manager_mjwarp_legacy_retirement.main(["--json"]) == 1
