"""Release-level freshness and fault tests for managed MuJoCo/MJWarp rollout."""
...
def test_malformed_final_artifact_fails_closed(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "tooling.acceptance.final_gate.validate_final_head",
...
def test_source_snapshot_rejects_executable_bit_changes(tmp_path: Path) -> None:
    ...
    subprocess.run(
        ("git", "config", "user.name", "managed MuJoCo/MJWarp rollout Test"),
        cwd=tmp_path,
        check=True,
    )
    subprocess.run(
        ("git", "config", "user.email", "manager_mjwarp@example.invalid"),
        cwd=tmp_path,
        check=True,
    )
...
def test_phase7_semantic_snapshot_allows_only_promotion_fields() -> None:
    raw = OmegaConf.to_container(
        OmegaConf.load(REPO_ROOT / "tests/acceptance/manager_mjwarp/manifests/phase_7.yaml"),
        resolve=False,
    )
