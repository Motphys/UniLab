"""Repository and synthetic fault audits for managed MuJoCo/MJWarp rollout support claims."""
...
import pytest
from tooling.acceptance import support_audit
from tooling.acceptance.commands import support as audit_manager_mjwarp_support
...
def test_support_audit_rejects_owner_registry_backend_and_profile_faults(
    ...
    discovered = support_audit._discover_mjwarp_owner_paths(REPO_ROOT)
    monkeypatch.setattr(
        manager_mjwarp_support,
...
def test_support_audit_rejects_manifest_receipt_that_differs_from_gate(
    ...
    manifest = _manifest()
    load_manifest = support_audit.load_phase_acceptance
    ...
    monkeypatch.setattr(manager_mjwarp_support, "load_phase_acceptance", load_with_stale_receipt)
...
def test_support_audit_cli_reports_pass_and_fail(
    ...
    monkeypatch.setattr(
        audit_manager_mjwarp_support, "audit_support_evidence", lambda *a, **k: passing
    )
    assert audit_support_audit.main([]) == 0
    assert "PASS managed MuJoCo/MJWarp rollout support audit" in capsys.readouterr().out

    failing = replace(passing, errors=("synthetic fault",))
    monkeypatch.setattr(
        audit_manager_mjwarp_support, "audit_support_evidence", lambda *a, **k: failing
    )
    assert audit_support_audit.main(["--json"]) == 1
