...
from tooling.acceptance.commands import thresholds as validate_manager_mjwarp_thresholds
...
def test_default_cli_validates_manifest_and_freeze_receipt(capsys) -> None:
    exit_code = validate_manager_mjwarp_thresholds.main(["--json"])
...
def test_manifest_only_cli_passes_and_reports_frozen_matrix(capsys) -> None:
    exit_code = validate_manager_mjwarp_thresholds.main(["--manifest-only", "--json"])
...
def test_manifest_only_cli_fails_on_threshold_relaxation(tmp_path: Path, capsys) -> None:
    raw = OmegaConf.load(
        validate_manager_mjwarp_thresholds.REPO_ROOT
        / validate_manager_mjwarp_thresholds.THRESHOLD_MANIFEST_PATH
    )
    raw.gates.performance.p50_latency_ratio_max = 1.06
    path = tmp_path / "thresholds.yaml"
    OmegaConf.save(raw, path)

    exit_code = validate_manager_mjwarp_thresholds.main(
        ["--manifest", str(path), "--manifest-only"]
    )
