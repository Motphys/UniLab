"""Repository and fault audits for the managed MuJoCo/MJWarp rollout task rollout plan."""
...
import pytest
from tooling.acceptance import task_rollout as manager_mjwarp_task_rollout
from tooling.acceptance.commands import task_rollout as audit_manager_mjwarp_task_rollout
from tooling.acceptance.support_audit import CompiledSignature, snapshot_registry_backends
...
def test_task_rollout_audit_rejects_prerequisite_and_phase_receipt_faults(
    ...
    load_manifest = manager_mjwarp_task_rollout.load_phase_acceptance
...
def test_task_rollout_audit_rejects_missing_exact_pytest_node(
    ...
    exists = manager_mjwarp_task_rollout.test_node_exists
    monkeypatch.setattr(
        manager_mjwarp_task_rollout,
        "test_node_exists",
...
def test_task_rollout_audit_cli_reports_pass_and_fail(
    ...
    monkeypatch.setattr(
        audit_manager_mjwarp_task_rollout,
        "audit_task_rollout_plan",
        lambda *args, **kwargs: passing,
    )
    assert audit_manager_mjwarp_task_rollout.main([]) == 0
    assert "PASS managed MuJoCo/MJWarp rollout task rollout audit" in capsys.readouterr().out
    ...
    assert audit_manager_mjwarp_task_rollout.main(["--json"]) == 1
