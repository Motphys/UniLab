...
def test_all_phase_manifests_exist_and_pass_schema() -> None:
    ...
    # Phase gates 0--3 have their independently captured evidence promoted in
    ...
    assert all(
        ...
        if manifest.phase in {0, 1, 2, 3}
        for claim in manifest.claims
    )
    assert all(
        claim.status == ClaimStatus.PLANNED
        for manifest in manifests
        if manifest.phase not in {0, 1, 2, 3}
        for claim in manifest.claims
    )
