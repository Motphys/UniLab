...
from tooling.acceptance.commands import claims as audit_manager_mjwarp_claims
from tooling.acceptance.commands import phase as validate_manager_mjwarp_phase
...
def test_all_phase_manifests_exist_and_pass_schema() -> None:
    manifests, errors = load_phase_manifests(audit_manager_mjwarp_claims.MANIFEST_DIR, PHASES)
...
def test_repository_claim_inventory_is_bidirectional_for_all_phases(capsys) -> None:
    exit_code = audit_manager_mjwarp_claims.main(["--all"])
...
def test_phase0_claim_inventory_cli_passes_json(capsys) -> None:
    exit_code = audit_manager_mjwarp_claims.main(["--phase", "0", "--json"])
...
def test_high_risk_claims_require_the_right_evidence_class() -> None:
    inventory = load_claim_gap_inventory(audit_manager_mjwarp_claims.INVENTORY_PATH)
    acceptance_by_claim = {
...
def test_phase_schema_cli_accepts_each_frozen_manifest(capsys) -> None:
    for phase in PHASES:
        assert validate_manager_mjwarp_phase.main(["--phase", str(phase), "--mode", "schema"]) == 0
    output = capsys.readouterr().out
...
def test_cli_fails_closed_when_manifest_is_missing(tmp_path: Path, monkeypatch, capsys) -> None:
    monkeypatch.setattr(audit_manager_mjwarp_claims, "MANIFEST_DIR", tmp_path / "manifests")

    exit_code = audit_manager_mjwarp_claims.main(["--phase", "0"])
...
def test_cli_fails_closed_when_inventory_is_malformed(tmp_path: Path, monkeypatch, capsys) -> None:
    ...
    monkeypatch.setattr(audit_manager_mjwarp_claims, "INVENTORY_PATH", inventory)
