...
class _DeployTerm:
    """Per-term history buffer with deque semantics.
...
class TestHistoryOrdering:
    ...
    def test_history_terms_carry_oldest_first(self, deploy_cfg, rng):
        """At step k>=H the 5*3 gyro slot should be
        [gyro_{k-H+1}, ..., gyro_k] flattened, NOT the reverse."""
...
class TestTrainingAssemblerVsDeploy:
    ...
    @staticmethod
    def _training_actor_obs(history_buf, current_refs, num_envs):
        parts = [
    ...
    def test_training_path_matches_deploy(self, deploy_cfg, rng):
        ...
        train_obs0 = self._training_actor_obs(buf, refs0, n_env)
        ...
        for k in range(1, len(all_segments)):
            ...
            train_obs = self._training_actor_obs(buf, refs, n_env)
...
class TestBackCompat:
    """H=1 ('no history') must reproduce the plain-concat 154-d path bit-exact."""
