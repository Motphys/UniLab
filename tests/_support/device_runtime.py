"""Shared real-CUDA harness for managed MuJoCo/MJWarp rollout device runtime acceptance tests."""
...
def require_cuda() -> None:
    ...
    if not bool(dependencies.warp.get_device().is_cuda):
        pytest.fail(
            "managed MuJoCo/MJWarp rollout device acceptance requires an active CUDA Warp device"
        )
    ...
    *,
    ...
    try:
        ...
        yield DeviceRuntimeHarness(
            ...
            action=action,
            action_lease=DeviceBufferLease("manager_mjwarp-stable-policy-action"),
