"""Freshness and tamper checks for the Issue #705 Phase 3 gate artifact."""
...
ARTIFACT_PATH = REPO_ROOT / "tests/acceptance/issue_705/artifacts/phase_3_gate.json"
MANIFEST_PATH = REPO_ROOT / "tests/acceptance/issue_705/manifests/phase_3.yaml"
ARTIFACT_REF = "tests/acceptance/issue_705/artifacts/phase_3_gate.json"


def test_phase3_gate_artifact_and_manifest_are_fresh_complete_and_promoted() -> None:
    report = load_phase3_evidence(ARTIFACT_PATH)
    assert validate_phase3_evidence(report, root=REPO_ROOT) == ()
    ...
    assert set(claims) == set(PHASE3_REQUIRED_TEST_IDS)
    assert set(artifact_claims) == set(PHASE3_REQUIRED_TEST_IDS)
    for claim_id, test_id in PHASE3_REQUIRED_TEST_IDS.items():
        ...
        assert artifact_claims[claim_id]["minimum_repetitions"] == PHASE3_MIN_REPETITIONS[claim_id]

    assert validate_issue705_phase.main(["--phase", "3", "--mode", "gate"]) == 0


def test_phase3_gate_artifact_faults_fail_closed() -> None:
    report = load_phase3_evidence(ARTIFACT_PATH)
    ...
    assert any(
        ...
        for error in validate_phase3_evidence(altered_command, root=REPO_ROOT)
    )
    ...
    assert any(
        ...
        for error in validate_phase3_evidence(incomplete_repetitions, root=REPO_ROOT)
    )
