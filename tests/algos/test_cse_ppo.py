...
def test_runner_iteration_summary_omits_timing_metrics(capsys: pytest.CaptureFixture[str]) -> None:
    from unilab.algos.cse_ppo import CSEOnPolicyRunner
    ...
    # CSE-PPO no longer consumes or prints per-iteration timing data.  The
    # shared environment timing contract remains available to other owners.
    output = capsys.readouterr().out
    assert "Computation (fps)" not in output
    assert "Iteration time" not in output
    assert "Iter waterfall" not in output
    assert "Time elapsed" not in output
    assert "ETA" not in output
    assert "Mean value loss" in output
